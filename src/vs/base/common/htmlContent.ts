...
import { escapeCodicons, unescapeCodicons } from 'vs/base/common/codicons';
...
export class MarkdownString implements IMarkdownString {
	...
	appendText(value: string): MarkdownString {
		...
		value = value
			.replace(/[\\`*_{}[\]()#+\-.!]/g, '\\$&')
			.replace('\n', '\n\n');
		this._value += this.supportThemeIcons ? unescapeCodicons(value) : value;

		return this;
	}

	appendMarkdown(value: string, { escapeThemeIcons }: { escapeThemeIcons?: boolean } = {}): MarkdownString {
		this._value += this.supportThemeIcons && escapeThemeIcons ? escapeCodicons(value, true) : value;
